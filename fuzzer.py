...
class Fuzz:
    ...
    def __init__(self, url, cookies, payloads):
        self.url = url
    ...
    def verify(self):
        if 'FUZZ' not in self.url:
            print(red + '\nThe keyword, "FUZZ," was not found in ' + self.url)
            exit()
        else:
            self.inject()
    ...
    def inject(self):
        if self.payloads:
            with open(self.payloads, 'r') as f_object:
                for payload in f_object.readlines():
                    payload = payload.strip('\n')
                    param = urllib.parse.quote(payload)  # encode payload
                    self.injectable_payloads.append(self.url.replace("FUZZ", param))
        else:
            with open("xssvectors.txt", 'r') as f_object:
                for payload in f_object.readlines():
                    payload = payload.strip('\n')
                    param = urllib.parse.quote(payload)  # encode payload
                    self.injectable_payloads.append(self.url.replace("FUZZ", param))
    ...
    def fuzz(self):
        ...
        for url in self.injectable_payloads:
            try:
                print('Testing ' + url)
                driver.get(url)
                if self.cookies:
